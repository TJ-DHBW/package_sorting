...
public class SortingFacilityProxy implements ISortingFacility {
    ...
    public SortingFacilityProxy(SortingFacility sortingFacility, String employeeType) {
        ...
        if(employeeClass == null) throw new IllegalStateException("A Proxy should not exist without knowing what privileges it has.");
    }
    ...
    private boolean checkPermission(int commandId){
        if(permissions.containsKey(employeeClass)){
            ...
            if(commandId > permissionArray.length || commandId < 1) throw new IllegalArgumentException("CommandId must be between 1 and " + permissionArray.length);
